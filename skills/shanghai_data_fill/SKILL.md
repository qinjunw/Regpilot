...
description: Portfolio-only simulation of a guarded Shanghai Data Platform fill contract; it never opens Chrome or writes a workbook.
regpilot_skill: true
---
...
This is one operator-facing Agent Skill for guarded Shanghai Data Platform filling. RegPilot may expose it as a business capability, but internal operation nodes remain backend orchestration details.
...
```json regpilot_manifest
{
  ...
  "title": "上海数据平台填报",
  "description": "根据工作簿、工作表和值所在列，在受控 Chrome 中执行上海数据平台受控填报，并停在保存/提交前。",
  "category": "fill",
  "status": "available",
  ...
  "operation_nodes": [
    ...
    },
    {
      ...
      "title": "点击下一步并确认",
      "boundary": "网页确认、重复型号等页面提示可能要求人工修正"
    },
    {
...
- Emit a Skill Command only when the operator clearly intends Shanghai Data Platform filling.
